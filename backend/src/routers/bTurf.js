...
router.get("/admin/bturfs", auth, async (req, res) => {
  // console.log(req);
  try {
    const user = await User.findById(req.user._id);
    if(!user.isAdmin) res.status(500).send('Not an Admin');
    const bturfs = await BTurf.find().populate('owner').sort({ date: 1 });
    res.status(200).json(bturfs);
  } catch (e) {
